  spa.avtr = do->
    ...
    onHeldmoveNav = ( event )->
      ...
      if not drag_map then return false
    ...
    onHeldendNav = ( event )->
      ...
      if not $drag_target then return false
      $drag_target
    ...
    onListchange = ( event )->
      ...
      $nav.empty()
      if user.get_is_anon() then return false

      people_db().each( ( person, idx )->
        if person.get_is_anon() then return true
